...
static void test_bas_init(void)
{
    ...
    zassert_true(k_sem_count_get(&batt_init_ok) == 1, "Initialization did not give the semaphore");
}
...
static void test_bas_timer_handler(void)
{
	...
	zassert_true(k_sem_count_get(&batt_ok) == 1, "Timer handler did not give the semaphore");
