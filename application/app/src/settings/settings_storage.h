 ...
 * This gets called to dump all current settings items. This happens when 
 * settings_save tries to save the settings.
 * @param cb Or export_func. The pointer to the internal function which 
 * appends a single key-value pair to persisted settings. Don’t store 
 ...
 * @param[in] name Or key. The name with skipped part that was used as name 
